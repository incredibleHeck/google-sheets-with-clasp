// Report Card Generator - Generates individual report cards from EOT Report sheet
// 61 lines
...
- Validates configuration and prompts user for IDs if needed
  ...
  let templateId = PropertiesService.getUserProperties().getProperty('RPT_TEMPLATE_ID');
  let folderId = PropertiesService.getUserProperties().getProperty('RPT_FOLDER_ID');
  let sheetName = PropertiesService.getUserProperties().getProperty('RPT_SHEET_NAME') || "PRIMARY EOT 1 REPORT";
...
ui.ButtonSet.OK_CANCEL
    ...
    // Save for future use
    PropertiesService.getUserProperties().setProperty('RPT_TEMPLATE_ID', templateId);
    PropertiesService.getUserProperties().setProperty('RPT_FOLDER_ID', folderId);
...
- Clears saved configuration (for resetting IDs)
  ...
  SpreadsheetApp.getUi().alert("Configuration cleared. Next run will prompt for IDs.", ui.ButtonSet.OK);
  }
